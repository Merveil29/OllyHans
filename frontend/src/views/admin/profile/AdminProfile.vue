        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow-sm overflow-hidden">
            <div class="p-6">
              ...
              <div class="flex flex-col items-center">
                ...
                <p class="mt-4 text-center text-sm text-gray-600">
                  Bienvenue <span class="font-semibold text-gray-900">{{ userProfile?.user_prenom }} {{ userProfile?.user_nom }}</span>
                </p>
                ...
                <div class="mt-6 w-full">
                  ...
                  <button
                    v-if="userProfile?.user_image && userProfile.user_image !== 'public/avatar.png' && !selectedFile"
            ...
            <div class="border-t border-gray-200 p-6">
              <h3 class="text-sm font-medium text-gray-900 mb-3">Informations du compte</h3>
              <div class="space-y-2">
                ...
                <div class="flex items-center justify-between text-sm">
                  ...
                  <span class="font-semibold text-gray-900">{{ userProfile?.user_login }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <form @submit.prevent="updateProfile" class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label for="user_nom" class="block text-sm font-medium text-gray-700 mb-2">
                      Nom
                    </label>
                    <input 
                      type="text" 
                      id="user_nom"
                      v-model="profileForm.user_nom"
                  ...
                  <div>
                    <label for="user_prenom" class="block text-sm font-medium text-gray-700 mb-2">
                      Prénom
                    </label>
                    <input 
                      type="text" 
                      id="user_prenom"
                      v-model="profileForm.user_prenom"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                  ...
                  <div class="md:col-span-2">
                    ...
                    <input 
                      type="email" 
                      id="user_email"
                      v-model="profileForm.user_email"
                      disabled
                  ...
                  <div>
                    <label for="user_tel" class="block text-sm font-medium text-gray-700 mb-2">
                      Téléphone
                    </label>
                    <input 
                      type="tel" 
                      id="user_tel"
                      v-model="profileForm.user_tel"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                  ...
                  <div>
                    ...
                    <input 
                      type="text" 
                      id="user_login"
                      v-model="profileForm.user_login"
                      disabled
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-500 cursor-not-allowed"
...
import * as adminProfileAPI from '@/api/modules/admin/profile'
...
const profileImage = computed(() => {
  if (!userProfile.value?.user_image || userProfile.value.user_image === 'public/avatar.png') {
    return '/avatar.png'
  }
  // Si l'image commence par http, c'est une URL Cloudinary
  if (userProfile.value.user_image.startsWith('http')) {
    return userProfile.value.user_image
  }
...
const fetchProfile = async () => {
  try {
    ...
    // Populate form
    profileForm.value = {
      user_nom: profileData.user_nom || '',
      user_prenom: profileData.user_prenom || '',
      user_email: profileData.user_email || '',
      user_login: profileData.user_login || '',
      user_tel: profileData.user_tel || ''
    }
  } catch (error) {
...
const updateProfile = async () => {
  ...
  try {
    const response = await adminProfileAPI.updateAdminProfile(profileForm.value)
    const profileData = response.data || response
    userProfile.value = profileData
    ...
    errorMessage.value = error.response?.data?.message || error.message || 'Erreur lors de la mise à jour du profil'
  } finally {
...
const uploadProfileImage = async () => {
  ...
  try {
    ...
    const response = await adminProfileAPI.updateAdminImage(formData)

    const imageUrl = response.data?.image_url || response.image_url
    if (userProfile.value) {
      userProfile.value.user_image = imageUrl
    }
...
const deleteProfileImage = async () => {
  ...
  try {
    await adminProfileAPI.deleteAdminImage()
    userProfile.value.user_image = 'public/avatar.png'
...
const changePassword = async () => {
  ...
  try {
    await adminProfileAPI.updateAdminPassword({
